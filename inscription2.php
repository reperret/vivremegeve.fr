...
//echo $output;echo "<br><br>";
...
) 
	{
		...
		else 
		{
			...
			foreach ($lignes as $colonne)
			{
				if ($colonne->nbre == 0   && checkDoublon("01-".$_POST['numResident']."-".$_POST['numResident2'],$_POST['nom'],$_POST['prenom'],$_POST['annee']."-".$_POST['mois']."-".$_POST['jour'],$dbh)==0) 
				{
					$idUtilisateur=NULL;
					
					
					//$dbh->beginTransaction();
					$reqInsert = $dbh->prepare("INSERT INTO utilisateur (email,pass_md5,nom,prenom,telephone,code_postal,adresse,ville, civilite, numResident, urlPhoto, ddn, typeAdherent, typePaiement, dateAdhesion, login, logNavigateur, renouvellement, montantAdhesion ) VALUES (?, ?, ?, ?, ?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		...
		La présidente, Annick SOCQUET CLERC.";
			...
			//******************** FIN GENERATION EMAIL CONFIRMATION CLIENT************************
				...
				{
					$erreur = 'Un membre possède déjà ce pseudonyme (auquel cas veuillez en choisir un autre), ou vous avez déjà un compte existant.';
				}	
			}	
