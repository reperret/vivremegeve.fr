...
if (!isset($_SESSION['login'])) {
	...
	exit();
		...
		if($_POST['pass']=="MPD_IMPROBABLEHUHUHU")
		{
			...
			$reqInsert = $dbh->prepare("UPDATE utilisateur SET email=?,nom=?,prenom=?,telephone=?,  civilite=?,adresse=?,code_postal=?,ville=?,ddn=? WHERE idUtilisateur=?");
			...
			$reqInsert->bindParam(10, $_POST['idToEdit'] );	
			
		}
		
		else
		{
			$reqInsert = $dbh->prepare("UPDATE utilisateur SET email=?,nom=?,prenom=?,telephone=?, pass_md5=?, civilite=?,adresse=?,code_postal=?,ville=?, ddn=?  WHERE idUtilisateur=?");
			$reqInsert->bindParam(1, $email);
			$reqInsert->bindParam(2, $nom);
			$reqInsert->bindParam(3, $prenom);
			$reqInsert->bindParam(4, $telephone);
