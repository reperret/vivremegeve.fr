            ...
            $loginExistant=verifierLoginExistant($_POST['login'],$dbh);
             ...
             echo "TEST10";
				...
				if($loginExistant==0  && $checkDoublon==0 && $codeMairie) 
				{
					...
					$reqInsert = $dbh->prepare("INSERT INTO utilisateur (email,pass_md5,nom,prenom,telephone,code_postal,adresse,ville, civilite, numResident, urlPhoto, ddn, typeAdherent, typePaiement, dateAdhesion, login, logNavigateur, renouvellement, montantAdhesion ,codeMairie) VALUES (?, ?, ?, ?, ?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
					...
					$reqInsert->bindParam(17, $logNavigateur);
					$reqInsert->bindParam(18, $renouvellement);
					$reqInsert->bindParam(19, $montantAnneeEnCours);
                    $reqInsert->bindParam(20, $codeRecupere);
					...
					if($choixPaiement=="cb")
					{
						$redirect='Location: payplug/paiementAdhesionCODE.php?idU='.$idUtilisateur.'&nom='.$nom.'&prenom='.$prenom.'&email='.$email.'&codeMairieGet='.$codeRecupere;
					}
					else
					{
		...
		La présidente, Annick SOCQUET CLERC.";
			...
			//******************** FIN GENERATION EMAIL CONFIRMATION CLIENT************************
				...
				else 
				{
                    if ($loginExistant==1) 
                    {
                        $erreur .= 'Un membre possède déjà ce pseudonyme (auquel cas veuillez en choisir un autre), ou vous avez déjà un compte existant.<br><br>';
                    }
                    if ($checkDoublon==1) 
                    {
                        $erreur .= 'Ce numéro de résident existe déjà<br><br>';
                    }
                    if($codeMairie==false)
                    {
                        $erreur .= 'Le code de la mairie que vous avez indiqué est invalide ou déjà utilisé. Veuillez le resaisir ou vérifier auprès de la mairie.<br><br>';
                    }
      ...
      function verifLogin(champ)
  {
	...
	for(x = 0; x < champ.value.length; x++)
	{
	    ...
	    if(champ.value.charAt(x) == "."){points++;}
            if(points > 1){verif = false; points = 1;}
  	    if(verif == false){champ.value = champ.value.substr(0,x) + champ.value.substr(x+1,champ.value.length-x+1); x--;}
		if(champ.value.length>35){champ.value = champ.value.substr(0,35) ;} 
...
if (isset($erreur)) 
{
	echo '<br /><br /><h1>',$erreur."</h1><br><br><br><br>";
	?>
         
	<h2>Vous pouvez retrouver vos anciens identifiants et mots de passe en cliquant <a href="http://www.vivremegeve.fr/demandeMdp.php">ici</a></h2><br><br><br><br> <?php	
}


?>
            <h1>Inscriptions 2020-2021</h1>
		...
		</div>    <center><u>Les informations ci dessous ne seront pas diffusées</u><br> Une inscription sur cette page correspondant à une et une seule personne physique. Vous pouvez créer différents comptes avec différents pseudonymes tout en utilisant une même adresse email.<br><br>
...
<form action="inscriptionCODE.php" method="post" class="pure-form pure-form-aligned">
    <fieldset>
 ...
 <br><br><br>
   ...
   <br><br><center>
          <span class="msg-error error"></span>
                    <div id="recaptcha" class="g-recaptcha" data-sitekey="6LflkJMUAAAAAFpk1PZYr2XOdnsigAjr9HMybi9m"></div></center>
                    <br><br> <br><br>
