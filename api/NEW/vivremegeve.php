...
function getUserVivreMegeve($numAdherent, $dbh)
{
    ...
    foreach ($lignes as $colonne) {
        ...
        // ➡️ Ajout des consentements RGPD
        $infosAdherent['rgpdVivreMegeve'] = (int)$colonne->rgpd_vivre_megeve;
        $infosAdherent['rgpdMairieMegeve'] = (int)$colonne->rgpd_mairie_megeve;
    }
...
function getUserVivreMegeve2($numAdherent, $nomAdherent, $dbh)
{
    ...
    foreach ($lignes as $colonne) {
        ...
        // ➡️ Ajout des consentements RGPD
        $infosAdherent[$i]['rgpdVivreMegeve'] = (int)$colonne->rgpd_vivre_megeve;
        $infosAdherent[$i]['rgpdMairieMegeve'] = (int)$colonne->rgpd_mairie_megeve;
        $i++;
    }
    return $infosAdherent;
}
...
if ($auth == "aiovbZRUOGTzbrvZRUGB,??452") {
    switch ($method) {
        ...
        case 'getUserVivreMegeve2':
            if (
                (isset($_POST['numAdherent']) && $_POST['numAdherent'] != '' ||  isset($_POST['nomAdherent']) && $_POST['nomAdherent'] != '')
            ) {
                echo json_encode(getUserVivreMegeve2($_POST['numAdherent'], $_POST['nomAdherent'], $dbh), JSON_FORCE_OBJECT | JSON_UNESCAPED_UNICODE);
            } else {
                $data = array();
                $data["codeErreur"]  = 'ERROR_MISSING_PARAMETERS';
                echo json_encode($data, JSON_FORCE_OBJECT | JSON_UNESCAPED_UNICODE);
