		...
		else 
		{
			...
			foreach ($lignes as $colonne)
			{
				if ($colonne->nbre == 0) 
				{
					...
					$reqInsert = $dbh->prepare("INSERT INTO utilisateur (email,pass_md5,nom,prenom,telephone,code_postal,adresse,ville, civilite, numResident, urlPhoto, ddn, typeAdherent, typePaiement, dateAdhesion, login, logNavigateur ) VALUES (?, ?, ?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<form action="inscription.php" method="post" class="pure-form pure-form-aligned">
    <fieldset>
      ...
      <div class="pure-control-group">
            ...
            <input type="text" placeholder="Pseudonyme" name="login" value="<?php 
